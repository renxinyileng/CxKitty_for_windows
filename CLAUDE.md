...
`SocialSisterYi/CxKitty`（超星学习通 TUI 自动化工具）的 Windows 分发 fork。上游程序本体在
`app/`，本 fork 额外提供各平台的环境准备脚本、CI 与启动器。

Python 运行时**不入库**，由 `scripts/setup-*` 按需下载：

| 目录 | 内容 | 是否入库 |
|---|---|---|
| `app/` | 程序本体（`main.py` / `cxapi` / `resolver` / `pyproject.toml` / `config.yml`） | 是 |
...
启动.bat                                   # Windows（缺 runtime 会自动安装）
...
验证码识别链路、人脸上传组包、题目 HTML 解析、试题导出、TUI 渲染、终端二维码。
