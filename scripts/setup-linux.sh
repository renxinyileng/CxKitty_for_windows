#!/usr/bin/env bash
#
# 为 CxKitty 准备 Linux 运行环境。
#
# python.org 只为 Windows 提供"嵌入式发行版", Linux 上没有对应的免安装包,
# 因此这里的做法是: 找一个可用的 Python (优先 3.13), 建立 .venv 并装好依赖。
# 若系统里一个合适的解释器都没有, 而机器上装了 uv, 就用 uv 下载一份独立构建的
# 最新 3.13 —— 这是 Linux 上最接近"自动下载指定版本"的免 root 方案。
#
# 用法:
...
没有找到 3.${MIN_MINOR} ~ 3.${MAX_MINOR} 范围内的 Python。任选一种方式装好后重新运行本脚本:
  ...
  # Debian / Ubuntu
  sudo apt install python${PYTHON_SERIES} python${PYTHON_SERIES}-venv

  # Fedora / RHEL
  sudo dnf install python${PYTHON_SERIES}

  # Arch
  sudo pacman -S python

  # 不想动系统环境 (免 root, 自动下载对应版本)
...
if [ ! -x "$VENV_DIR/bin/python" ]; then
    ...
    "$PYTHON" -m venv "$VENV_DIR" || die "创建 venv 失败; Debian/Ubuntu 上可能需要先装 python${PYTHON_SERIES}-venv"
else
